<template>
  <v-container class="grey lighten-3 ml-5">
    <v-row no-gutters>
      <!-- <v-col cols="12" sm="4">
        <v-card class="pa-3" outlined tile>
          <v-card max-width="344" class="mx-auto" flat>
            <v-card-title>I'm a title</v-card-title>
            <v-card-text>
              <v-card-text>
                <span>Number 10</span>
                <br />
                <span class="text--primary">
                  <span>Whitehaven Beach</span>
                  <br />
                  <span>Whitsunday Island, Whitsunday Islands</span>
                </span>
              </v-card-text>
            </v-card-text>
            <v-card-actions>
              <v-btn text>Click</v-btn>
            </v-card-actions>
          </v-card>
        </v-card>
      </v-col>-->
      <v-col cols="12" sm="12">
        <v-card class="pa-4 py-5 my-1" flat>
          <v-card class="pa-4" flat>
            <v-card max-width="550" class="mx-auto" flat>
              <v-card-title>A rule isn't unfair if it applies to everyone</v-card-title>
              <v-card-text>
                <v-card-text>
                  <span>Number 10</span>
                  <br />
                  <span class="text--primary">
                    <span>Whitehaven Beach</span>
                    <br />
                    <span>Whitsunday Island, Whitsunday Islands</span>
                  </span>
                </v-card-text>
              </v-card-text>
              <v-card-actions>
                <v-chip class="ma-3" color="indigo darken-3" outlined tile>
                  <v-icon left>mdi-fire</v-icon>Read More Our Services
                </v-chip>
              </v-card-actions>
            </v-card>
          </v-card>
        </v-card>
      </v-col>
      <!-- <v-col cols="12" sm="4">
        <v-card class="pa-2" outlined tile>
          <v-card class="pa-3" outlined tile>
            <v-card max-width="344" class="mx-auto" flat>
              <v-card-title>I'm a title</v-card-title>
              <v-card-text>I'm card text</v-card-text>
              <v-card-actions>
                <v-btn text>Click</v-btn>
              </v-card-actions>
            </v-card>
          </v-card>
        </v-card>
      </v-col>-->
    </v-row>
  </v-container>
</template>